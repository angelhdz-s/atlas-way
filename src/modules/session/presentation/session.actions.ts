...
export async function createSessionAction(
  ...
  const exerciseIds: ExerciseProps['id'][] | null =
    sessionFormData.exercises &&
    sessionFormData.exercises.length > 0
      ? sessionFormData.exercises.map(
...
export async function createSession(
  ...
  const createSession =
    container.session.CreateSessionUseCase;
  const sessionResult = await createSession.execute(
    sessionData,
    exerciseIds
  );
  if (!sessionResult.success)
    return ActionFailure(sessionResult.error.message);
