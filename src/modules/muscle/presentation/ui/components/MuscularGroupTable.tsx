    ...
    <div className="bg-middle border-bd-default rounded-xl border">
      <table className="w-full">
        <thead className="h-10 divide-y">
          <tr className="fg-muted font-light">
        ...
        <tbody className="divide-bd-default divide-y">
          {muscularGroups.map(({ name, muscles, section }, index) => (
            <tr key={name} className="first:border-bd-default h-12 first:border-t">
              <td className="fg-muted pl-4 font-light">{index + 1}</td>
              <td className="fg-strong">{name}</td>
              <td className="text-sm">
                <div className="bg-front w-fit rounded-full px-3 py-1 font-light">
                  {section.name}
                </div>
              </td>
              <td className="text-sm">
                <div className="hidden w-fit items-center gap-2 lg:flex">
                  {muscles.map((m) => (
                    <div
                      key={m}
                      className="bg-front mx-auto w-fit rounded-full px-3 py-1 font-light"
                    >
                      {m}
                    </div>
                  ))}
