...
import type { ExerciseToMuscle } from '@/modules/exercise/link/muscle/domain/exercise-to-muscle.entity';
import type { MuscleProps } from '../domain/muscle.types';

export async function getAllMuscles(): ActionResponse<
  MuscleDTO[]
> {
  const container = getContainer();
  const getAllMuscles =
    container.muscle.GetAllMusclesUseCase;
  const musclesResult = await getAllMuscles.execute();
  if (!musclesResult.success)
    return ActionFailure(musclesResult.error.message);
  const musclesDTO = musclesResult.data.map((muscle) =>
    MuscleMapper.toDTO(muscle)
  );
  return ActionSuccess(
    musclesDTO,
    'Muscles were obtained successfully'
  );
}

export async function getMuscleById(
  id: MuscleProps['id']
): ActionResponse<MuscleDTO | null> {
  const container = getContainer();
  const getMuscleById =
    container.muscle.GetMuscleByIdUseCase;
...
export async function getAllMusclesByExercise(
  ...
  const getExercisesToMusclesByExerciseId =
    container.exerciseToMuscle
      .GetExercisesToMusclesByExerciseIdUseCase;
  const exerciseToMusclesResult =
    await getExercisesToMusclesByExerciseId.execute(
      exerciseId
    );
  if (!exerciseToMusclesResult.success)
    return ActionFailure(
      exerciseToMusclesResult.error.message
    );
  ...
  for (const { muscleId } of exerciseToMuscles) {
    ...
    if (!muscleRequest.success)
      return ActionFailure(muscleRequest.message);
    if (!muscleRequest.data)
      return ActionFailure(
        `Muscle with id ${muscleId} was not found`
      );
