...
import type { CreateExerciseInitialStatsWithoutExerciseIdInput } from '@/modules/exercise-initial-stats/application/dtos/create-exercise-initial-stats.dto';
...
export async function createExerciseAction(
  ...
  const exerciseParseResult =
    ExerciseFormSchema.safeParse(data);
  ...
  if (!userIdResult.success || !userIdResult.data)
    return ActionFailure(userIdResult.message);
  const userId = userIdResult.data;

  const container = getContainer();
  const createExercise =
    container.exercise.CreateExerciseUseCase;

  const exercise: CreateExerciseInput = {
    name: parsedExercise.exercise.name,
    description:
      parsedExercise.exercise.description ?? null,
    userId,
  };

  const muscles = parsedExercise.muscles.map(({ id }) =>
    Number(id)
  );

  if (!parsedExercise.initialStats) {
    const createdExercise = await createExercise.execute(
      exercise,
      muscles
    );
    if (!createdExercise.success)
      return ActionFailure(createdExercise.error.message);
  ...
  const exerciseDTO = ExerciseMapper.toDTO(
    createdExercise.data
  );
...
export async function getAllUserExercises(
  ...
  const getAllExercises =
    container.exercise.GetAllUserExercisesUseCase;
  const exercisesResult =
    await getAllExercises.execute(userId);
  if (!exercisesResult.success)
    return ActionFailure(exercisesResult.error.message);
  const exercises = exercisesResult.data;

  const fullExercises: FullExerciseDTO[] = [];

  for (const exercise of exercises) {
    const musclesResult = await getAllMusclesByExercise(
      exercise.id
    );
    if (!musclesResult.success)
      return ActionFailure(musclesResult.message);
  ...
  return ActionSuccess(
    fullExercises,
    'User exercises found successfully'
  );
}
